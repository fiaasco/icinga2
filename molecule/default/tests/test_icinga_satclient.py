...
testinfra_hosts = testinfra.utils.ansible_runner.AnsibleRunner(
    os.environ['MOLECULE_INVENTORY_FILE']).get_hosts('icinga2satclient')


def test_sat_client_config(host):
    ...
    with host.sudo():
        ...
        assert file.exists
        # not nagios here for some reason:
        ...
        # Check if satellite endpoint is configured
        endpoints = host.run('icinga2 object list --type endpoint')
        assert "-icinga2-satellite' of type 'Endpoint':" in endpoints.stdout
...
def test_sat_client_connection(host):
